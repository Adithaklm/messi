...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("**ഒന്ന് പോടാപ്പാ** ❗️", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('ഫയൽ ഞാൻ നിങ്ങളുടെ 𝗣𝗠ൽ സെൻട് ചെയ്തിട്ടുണ്ട്', show_alert=True)
        ...
        buttons = [[
            InlineKeyboardButton('Mᴀɴᴜᴀʟ ғɪʟᴛᴇʀ 🔧', callback_data='manuelfilter'),
            InlineKeyboardButton('Aᴜᴛᴏ ғɪʟᴛᴇʀ 🛠', callback_data='autofilter')
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇꜱ', url='https://t.me/fh_bot_updates'),
            InlineKeyboardButton('👨‍💻ᴅᴇᴠ', url='https://t.me/my_devs')
        ], [
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap =script.IDM_TEXT.format(search, temp.B_NAME, temp.B_NAME) 
    if imdb and imdb.get('poster'):
        ...
        await message.reply_text(tic, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
    if spoll:
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply("നിങ്ങൾ സേർച്ച് ആക്കിയ മൂവി താഴെ കാണുന്ന ലിസ്റ്റില് ഉണ്ടെങ്കിൽ\nനിങ്ങൾക്ക് വേണ്ട സൈസ് നോക്കി ബട്ടൺ പ്രസ് ചെയ്യുക?",
                    reply_markup=InlineKeyboardMarkup(btn))
